...
public class EventsManager {
    ...
    private CheckConditionsResult checkConditions(ConditionEvent conditionEvent, boolean isPlaceholderAPI) {
        List<String> conditions = new ArrayList<>(conditionEvent.getCurrentEvent().getConditions());
        ...
        for (int i = 0; i < conditions.size(); i++) {
            ...
            StringBuilder conditionLineWithReplacements = new StringBuilder();

            //AND
            //If a miniCondition is NOT achieved, it will finish this cycle.
            //If a miniCondition is achieved, it will check the next.

            //OR
            //If a miniCondition is NOT achieved, it will check the next.
            //If a miniCondition IS achieved, it will finish this cycle.
            //If this cycle is finished with NO achieved miniConditions, the
            ...
            for (int c = 0; c < separatedConditions.length; c++) {
                ...
                for (ConditionalType conditionalType : ConditionalType.values()) {
                    ...
                    if (miniCondition.contains(textToFind)) {
                        ...
                        conditionLineWithReplacements.append("'").append(firstArg).append("'").append(textToFind).append("'").append(secondArg).append("'");
                        if (c != separatedConditions.length - 1) {
                            if (separatorType.equals(SeparatorType.OR)) {
                                conditionLineWithReplacements.append(" or ");
                            } else if (separatorType.equals(SeparatorType.AND)) {
                                conditionLineWithReplacements.append(" and ");
                            }
                        }
                        ...
                        switch (conditionalType) {
                            case EQUALS:
                            case EQUALS_LEGACY:
                                if (firstArg.equals(secondArg)) approvedLine = true;
                                break;
                            case NOT_EQUALS:
                            ...
                            case EQUALS_IGNORE_CASE:
                                if (firstArg.equalsIgnoreCase(secondArg)) approvedLine = true;
                                break;
                            case NOT_EQUALS_IGNORE_CASE:
                                if (!firstArg.equalsIgnoreCase(secondArg)) approvedLine = true;
                                break;
                            case STARTS_WITH:
                                if (firstArgLower.startsWith(secondArgLower)) approvedLine = true;
                                break;
                            case NOT_STARTS_WITH:
                                if (!firstArgLower.startsWith(secondArgLower)) approvedLine = true;
                                break;
                            case CONTAINS:
                                if (firstArgLower.contains(secondArgLower)) approvedLine = true;
                                break;
                            case NOT_CONTAINS:
                                if (!firstArgLower.contains(secondArgLower)) approvedLine = true;
                                break;
                            case ENDS_WITH:
                                if (firstArgLower.endsWith(secondArgLower)) approvedLine = true;
                                break;
                            case NOT_ENDS_WITH:
                                if (!firstArgLower.endsWith(secondArgLower)) approvedLine = true;
                                break;
                            case MATCHES:
                                if (firstArgLower.matches(secondArgLower)) approvedLine = true;
                                break;
                            case NOT_MATCHES:
                                if (!firstArgLower.matches(secondArgLower)) approvedLine = true;
                                break;
    ...
    public boolean checkToConditionAction(List<String> conditionGroup, Player player, boolean isPlaceholderAPI,
        ...
        for (String conditionLine : conditionGroup) {
            boolean approvedLine = false;
